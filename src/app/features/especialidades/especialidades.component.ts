...
import { Especialidad, EspecialidadCreate, EspecialidadUpdate } from '../../shared/models';
...
export class EspecialidadesComponent implements OnInit {
  private readonly service = inject(EspecialidadService);
  ...
  readonly selectedItem = signal<Especialidad | null>(null);
  readonly confirmDeleteOpen = signal(false);
  readonly itemToDelete = signal<Especialidad | null>(null);
  ...
  trackById(_: number, item: Especialidad): number {
    return item.id;
  }
  ...
  startEdit(item: Especialidad): void {
    this.error.set(null);
  ...
  promptDelete(item: Especialidad): void {
  ...
  confirmDelete(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: () => {
          this.error.set('No se pudo eliminar la especialidad. Inténtalo nuevamente.');
        }
      });
  }
  ...
  private buildPayload(): EspecialidadCreate | EspecialidadUpdate {
  ...
  private handleFormError(error: unknown): void {
    this.error.set('No se pudo guardar la especialidad. Revisa los errores e inténtalo nuevamente.');
